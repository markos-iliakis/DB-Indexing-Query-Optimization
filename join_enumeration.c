		...
		for (int j = 0; j < n_tables; j++) {
			...
			for (int m = 0; m < n_tables; m++) {
				...
				if (!connected(q, s_rels, i, r_rels) || (r_in_s(s_rels, i, r_rels)))
					...
					int cost1 = bt[nh].cost; // cost(BestTree(union))
					int cost2 = cost_join(new_s_rels); // cost(currTree)

					if (bt[nh].jeT == NULL || cost1 > cost2) {
						//BestTree(union) = currTree
						bt[nh].jeT = malloc((i+1)*sizeof(int));
						for (int l = 0; l < i+1; l++)
							bt[nh].jeT[l] = new_s_rels[l] ;
